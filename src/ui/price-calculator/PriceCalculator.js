...
class PriceCalculator extends React.Component {
    ...
    loadPriceResult = (modelId , postcode , division) => {
        return fetch(encodeURI(`${REACT_APP_API_HOST}/vpc/price?postcode=${postcode}&modelid=${modelId}&divisionid=${division}`),
            {
                method: "GET",
    ...
    loadHTTPData = () => {
        ...
        return fetch(encodeURI(`${REACT_APP_API_HOST}/vpc/models?brandid=[${this.props.brand.id}]`))
            .then((response) => {
                if (response.ok) {
                    return response.json();
                }
        ...
        return (
            <div className="container-full" style={{backgroundColor:"#e7e7e7"}}>
                ...
                <div className="headerRow container-full" style={styles.header}>
                    <HeaderFrom
                        onSubmit={(values, dispatch, props) => {
                            const modelId = props.models[props.selection.model]["model_configurations"][props.selection.specification]["id"];
                            return this.loadPriceResult(modelId, values.postcode , division)
                                .then(result => {
                ...
                <div className="container-full main">
                    ...
                    {serverResponse && <BodyComponent
                        ref={(node) => {this._sivNode = node;}} make={this.props.models[this.props.selection.model].model}
                        info={Object.assign({ }, this.props.models[this.props.selection.model]["model_configurations"][this.props.selection.specification], this.props.selection.priceinfo)}/>}
